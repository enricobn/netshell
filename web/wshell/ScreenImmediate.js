function Screen(canvas_id) {
    ...
    this.back_ctx.lineWidth = 2;
    this.font_base = '16px Courier New';
    
    // I must apply attributes (font basically) to calculate font size 
    this.default_cell_attributes.apply(this.back_ctx, this.font_base);

    this.wrap_around = false;
    this.cell_width = this.back_ctx.measureText('M').width;
    ...
    this.back_canvas.width = this.cell_width * this.width;
    this.back_canvas.height = this.cell_height.height * this.height;
    this.back_canvas.visible = false;
    
    this.canvas.width = this.back_canvas.width;
    this.canvas.height = this.back_canvas.height;

    this.ctx.fillStyle = this.default_cell_attributes.fg_color;
    
    this.tmp_canvas = document.createElement('canvas');
    this.tmp_canvas.width = this.back_canvas.width;
    this.tmp_canvas.height = this.back_canvas.height;
    ...
    // for cursor
    this.back_ctx.strokeStyle = this.default_cell_attributes.fg_color;
    ...
    this.back_ctx_cell_attributes = undefined;
...
Screen.prototype.back_canvas;
Screen.prototype.back_ctx;
...
Screen.prototype.scroll_back_from = function() {
    this.flush();
    ...
    for (var y = 0; y < this.height; y++) {
        ...
        this.back_ctx.fillText(line, 0, y * this.cell_height.height + 
...
Screen.prototype.flush = function() {
    ...
    if (this.colors) {
        this.apply_cell_attributes(this.cell_attributes);
    }
    
    this.back_ctx.clearRect(this.cursor.x * this.cell_width, this.cursor.y * this.cell_height.height,
            this.cell_width * buffer.length, this.cell_height.height);
    this.back_ctx.fillText(buffer, this.cursor.x * this.cell_width, this.cursor.y * this.cell_height.height + 
            this.cell_height.ascent);
    var s = '';
    if (this.cursor.x > 0) {
...
Screen.prototype.add_char = function (c) {
    if (!this.equals_attributes(this.cell_attributes, this.back_ctx_cell_attributes)) {
        this.flush();
    }
...
Screen.prototype.apply_cell_attributes = function(attributes) {
    this.flush();
    
    if (!this.equals_attributes(attributes, this.back_ctx_cell_attributes)) {
        if (attributes) {
            attributes.apply_diff(this.back_ctx, this.font_base, this.back_ctx_cell_attributes);
        } else {
            this.default_cell_attributes.apply_diff(this.back_ctx, this.font_base, this.back_ctx_cell_attributes);
        }
    } 
/*
    if (attributes) {
        if (this.back_ctx_cell_attributes || !attributes.equals(this.default_cell_attributes)) {
            attributes.apply_diff(this.back_ctx, this.font_base, this.back_ctx_cell_attributes);
        }
    } else {
        if (this.back_ctx_cell_attributes && !this.back_ctx_cell_attributes.equals(this.default_cell_attributes)) {
            this.default_cell_attributes.apply_diff(this.back_ctx, this.font_base, this.back_ctx_cell_attributes);
        }
    }
*/
    this.back_ctx_cell_attributes = attributes;
};
...
Screen.prototype.set_char = function (c, attributes) {
    ...
    if (this.cursor.x <= this.width) {// && this.cursor.y < this.height) {
        ...
        if (this.colors) {
            this.apply_cell_attributes(attributes);
        }
        this.back_ctx.clearRect(this.cursor.x * this.cell_width, this.cursor.y * this.cell_height.height,
            this.cell_width, this.cell_height.height);
        this.back_ctx.fillText(c, this.cursor.x * this.cell_width, this.cursor.y * this.cell_height.height + 
            this.cell_height.ascent);
        if (this.cursor.x == this.width) {
...
Screen.prototype.redraw_line = function (line) {
    this.flush();
    this.back_ctx.clearRect(0, line * this.cell_height.height, this.back_canvas.width, (line + 1) * this.cell_height.height);
    this.draw_line(line);
};
...
Screen.prototype.draw_line = function (line) {
    this.flush();
    if (this.colors) {
        ...
        for (var x = 0; x < this.width; x++) {
            ...
            if (!this.equals_attributes(attributes, this.back_ctx_cell_attributes)) {
                this.back_ctx.fillText(this.cells[line].substr(current_x, x -current_x), current_x * this.cell_width, line * this.cell_height.height + 
                    this.cell_height.ascent);
                this.apply_cell_attributes(attributes);
                current_x = x;
            }
        ...
        this.back_ctx.fillText(this.cells[line], 0, line * this.cell_height.height + 
            this.cell_height.ascent);
    }
};
...
Screen.prototype.insert_chars = function (count) {
    ...
    this.redraw_line(this.cursor.y);
...
Screen.prototype.hide_cursor = function() {
    if (this.cursor_visible) {
        this.flush();
        if (this.cursor.x <= this.width) {
            // -1 since (I think) the size of stroke is 2
            this.back_ctx.clearRect(this.cursor.x * this.cell_width -1, this.cursor.y * this.cell_height.height, this.cell_width + 1, 
                this.cell_height.height);
            // since set_char calls set_cursor going in an infinite loop 
            // I'm outside the window is useless to set a char outside the window 
            if (this.cursor.x < this.width) {
                var attributes = undefined;
                if (this.colors) {
                    attributes = this.cells_attributes[this.cursor.y][this.cursor.x];
                }
                this.set_char(this.cells[this.cursor.y].charAt(this.cursor.x), attributes);
            }
            this.updated = false;
        }
...
Screen.prototype.show_cursor = function() {
    if (!this.cursor_visible) {
	    this.flush();
        if (this.cursor.x <= this.width) {
            this.back_ctx.globalCompositeOperation="xor";
            this.back_ctx.fillRect(this.cursor.x * this.cell_width, this.cursor.y * this.cell_height.height,
                this.cell_width, this.cell_height.height);
            this.back_ctx.globalCompositeOperation="source-over";
            /*
            this.back_ctx.beginPath();
            this.back_ctx.moveTo(this.cursor.x * this.cell_width + 1, this.cursor.y * this.cell_height.height);
            this.back_ctx.stroke();
            */
            this.updated = false;
        }
        this.cursor_visible = true;
...
Screen.prototype.erase_line_from_cursor = function () {
    ...
    this.back_ctx.clearRect(this.cursor.x * this.cell_width -1, this.cursor.y * this.cell_height.height, this.back_canvas.width, 
            this.cell_height.height);
};
